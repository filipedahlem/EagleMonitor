...
if(empty($dados['email'])){
    ...
    $query_usuario = "SELECT id, nome, email, senha, telefone, sits_usuario_id, recuperar_senha
                FROM usuarios
                WHERE email=:email
                LIMIT 1";
    ...
    if(($result_usuario) and ($result_usuario->rowCount() != 0)){
        ...
        if($row_usuario['sits_usuario_id'] != 1){
            $retorna = ['erro'=> true, 'msg' => "<div class='alert alert-danger' role='alert'>Erro: Necessário confirmar o e-mail!</div>"];
